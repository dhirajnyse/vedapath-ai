(function () {
  ...
  function controlledPermissionExecutionAuthorizationReviewDecisionGate(config, reviewPacket, decision) {
    if (!draftReviewPacketReady(reviewPacket, config)) {
      return blocked("Blocked: draft-review packet must be the v3.6.5 non-authorizing review packet.", {
    ...
    if (!decisionPreservesHandoff(decision, reviewPacket, config)) {
      return blocked("Blocked: decision must preserve the v3.6.5 route, questions, founder posture id, source ids, and authority audit.", {
    ...
    if (!compact(decision.decision_scope).includes("v3.6.5") ||
      ...
      return blocked("Blocked: decision text must name the v3.6.5 handoff, founder posture id, source identity, and authority audit.", {});
    }
  ...
  function loadConfig(config) {
    ...
    renderList("reviewDecisionScope", [
      { label: "Input", value: "v3.6.5 draft-review packet" },
