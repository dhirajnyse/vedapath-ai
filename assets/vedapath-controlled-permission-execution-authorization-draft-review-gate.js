(function () {
  ...
  function controlledPermissionExecutionAuthorizationDraftReviewGate(config, draftPacket, review) {
    if (!draftPacketReady(draftPacket, config)) {
      return blocked("Blocked: controlled draft packet must be the v3.8.0 non-authorizing draft candidate.", {
    ...
    if (!reviewPreservesHandoff(review, draftPacket, config)) {
      return blocked("Blocked: review must preserve the v3.8.0 source identity, founder posture id, route, questions, and authority audit.", {
    ...
    if (!compact(review.review_scope).includes("v3.8.0") ||
      ...
      return blocked("Blocked: review text must name the v3.8.0 handoff, source identity, and authority audit.", {});
    }
  ...
  function loadConfig(config) {
    ...
    renderList("draftReviewScope", [
      { label: "Input", value: "v3.8.0 draft packet" },
