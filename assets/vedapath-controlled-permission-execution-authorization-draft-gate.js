(function () {
  ...
  function controlledPermissionExecutionAuthorizationDraftGate(config, decisionPacket, draft) {
    if (!founderDecisionPacketReady(decisionPacket, config)) {
      return blocked("Blocked: founder decision packet must be the v3.8.7 draft-only, non-authorizing posture packet.", {
    ...
    if (!draftPreservesCarry(draft, decisionPacket, config)) {
      return blocked("Blocked: draft must preserve the v3.8.7 source ids, review route, questions, and authority audit.", {
    ...
    if (!compact(draft.draft_scope).includes("v3.8.7") ||
        !hasText(draft.draft_rationale, [["v3.8.7"], ["question handoff"], ["authority flag audit"], ["source ids"], ["draft review gate"], ["not a live authorization"]]) ||
        !compact(draft.draft_evidence_summary).includes("authority flag audit")) {
      return blocked("Blocked: draft text must name the v3.8.7 handoff, source ids, draft review gate, and authority audit.", {});
    }
  ...
  function loadConfig(config) {
    ...
    renderList("draftGateScope", [
      { label: "Input", value: "v3.8.7 founder posture" },
