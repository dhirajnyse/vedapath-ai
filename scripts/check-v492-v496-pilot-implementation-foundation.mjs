...
function finalChecks() {
  ...
  assert(new RegExp('const releaseBadge = "(?:v4\\.9\\.6 pilot implementation gate|v4\\.9\\.7 infrastructure authorization|v4\\.9\\.8 deployment contract|v4\\.9\\.9 identity contract|v5\\.(?:0\\.[0-9]|1\\.[0-6]) [^"]+)";').test(shell), "compatible release badge");
  ...
  assert(/<strong>(?:v4\.9\.[6-9]|v5\.(?:0\.[0-9]|1\.[0-6]))<\/strong>/.test(text("build-status.html")), "build status compatible version");
  assert(/(?:infrastructure authorization|private infrastructure implementation|private invitation issuance|audited pilot execution|private-pilot operations)/i.test(text("build-status.html")), "build status next decision");
