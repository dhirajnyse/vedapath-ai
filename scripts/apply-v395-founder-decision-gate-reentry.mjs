...
const sampleReviewDecisionPacket = {
  ...
  input_release: reviewData.input_release || "v3.9.3",
...
const founderDecision = {
  ...
  founder_permission_execution_authorization_decision_id:
    "founder-permission-execution-authorization-decision-v395",
  release,
  ...
  founder_decision_language:
    `I approve only the drafting of a controlled authorization candidate from the ${previousRelease} review-decision packet. This is not permission, not execution, not storage approval, not public release, and not production authorization.`,
  decision_rationale:
...
founderJs = replaceEvery(founderJs, '"v3.9.0"', `"${previousRelease}"`);
founderJs = replaceEvery(founderJs, "v3.9.0", previousRelease);
founderJs = replaceEvery(founderJs, "v3.9.1", release);
founderJs = replaceEvery(founderJs, "the v3.9.4 route", `the ${previousRelease} route`);
write(paths.founderJs, founderJs);

let page = read(paths.founderPage);
page = replaceEvery(page, "v3.9.1 founder", "v3.9.5 founder");
page = replaceEvery(page, "v3.9.1", release);
page = replaceEvery(page, "v3.9.0", previousRelease);
page = replaceEvery(page, "Record posture. Open no gate.", "Choose posture. Grant nothing.");
page = replaceEvery(
  page,
...
page = replaceEvery(page, "v3.9.4 review decision", `${previousRelease} review decision`);
write(paths.founderPage, page);

let css = read(paths.founderCss);
const cssAppend = `

/* VEDAPATH v3.9.5 FOUNDER POSTURE QUIET DESK */
.founder-decision .founder-decision-head h1 {
  max-width: 760px;
  font-size: clamp(2rem, 4vw, 4.2rem);
  line-height: 1.03;
}

.founder-decision .muted,
.founder-decision-form label,
.founder-decision-output p,
...
.founder-decision-meridian article,
.founder-decision-form,
.founder-decision-output,
.founder-decision-card {
  background: rgba(255, 252, 246, 0.82);
  border-color: rgba(168, 62, 18, 0.14);
  box-shadow: 0 18px 50px rgba(80, 44, 22, 0.055);
}

.founder-decision-choice[aria-pressed="true"] {
  background: linear-gradient(180deg, rgba(255, 241, 232, 0.95), rgba(255, 252, 246, 0.88));
  border-color: rgba(214, 90, 31, 0.42);
}

.founder-decision-meridian {
  align-items: stretch;
}

.founder-decision-meridian strong,
...
`;
if (!css.includes("VEDAPATH v3.9.5 FOUNDER POSTURE QUIET DESK")) {
  css += cssAppend;
}
...
commandShell = replaceOrVerify(
  ...
  'const releaseBadge = "v3.9.5 founder";',
  'const releaseBadge = "v3.9.5 founder";',
  "command shell badge"
);
...
build = replaceOrVerify(
  build,
  '<span class="version">v3.9.4 decision</span>',
  '<span class="version">v3.9.5 founder</span>',
  '<span class="version">v3.9.5 founder</span>',
  "build version badge"
);
build = replaceOrVerify(
  build,
  /<strong>v3\.9\.4<\/strong>\s*<p>Controlled Permission Execution Authorization Review Decision Gate Re-entry:[\s\S]*?<\/p>/,
...
build = replaceOrVerify(
  build,
  "The trust chain now routes the v3.9.3 draft-review packet as one calm handoff while preserving question handoff, source identity, founder posture id, authority flag audit, and all false execution flags.",
...
build = replaceOrVerify(
  build,
  /<strong>Founder Permission Execution Authorization Decision Gate Re-entry<\/strong>\s*<p>Receive the v3\.9\.4 review-decision packet and record founder posture while every authority flag remains false\.<\/p>/,
...
build = replaceOrVerify(
  build,
  /<article class="phase">\s*<span class="badge later">Later<\/span>\s*<div>\s*<strong>Phase 355: Founder Permission Execution Authorization Decision Gate Re-entry<\/strong>\s*<p>Receive the v3\.9\.4 review-decision packet and record founder posture while every authority flag remains false\.<\/p>\s*<\/div>\s*<div class="percent">0%<\/div>\s*<\/article>/,
  `<article class="phase">
  ...
  "Phase 356: Controlled Permission Execution Authorization Draft Gate Re-entry",
  "build roadmap phase"
);
build = replaceOrVerify(
  build,
  /<div class="version-row"><span>Release<\/span><strong>v3\.9\.4 Controlled Permission Execution Authorization Review Decision Gate Re-entry<\/strong><\/div>\s*<div class="version-row"><span>Previous<\/span><strong>v3\.9\.3 Controlled Permission Execution Authorization Draft Review Gate Re-entry<\/strong><\/div>\s*<div class="version-row"><span>Commit<\/span><strong>See GitHub history<\/strong><\/div>\s*<div class="version-row"><span>Goal<\/span><strong>[\s\S]*?<\/strong><\/div>\s*<div class="version-row"><span>Status<\/span><strong>Ready for founder decision gate re-entry<\/strong><\/div>/,
