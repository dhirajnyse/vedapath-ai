...
const release = "v3.8.7";
const inputRelease = "v3.8.6";
const releaseName = "Founder Permission Execution Authorization Decision Gate Re-entry";
const releaseBadge = "v3.8.7 founder";
const previousRelease = "v3.8.6 Controlled Permission Execution Authorization Review Decision Gate Re-entry";
...
founderData.generated_at = "2026-07-03";
...
gateJs = gateJs.replaceAll("v3.8.2", inputRelease);
...
page = page.replaceAll("v3.8.3 founder", releaseBadge);
page = page.replaceAll("v3.8.3", release).replaceAll("v3.8.2", inputRelease);
page = page.replace("Choose posture. Keep product still.", "Choose posture. Preserve the calm.");
page = page.replace("Choose posture. Keep authority closed.", "Choose posture. Grant nothing.");
page = page.replace("Choose posture. Preserve the calm.", "Choose posture. Grant nothing.");
page = page.replace(
  `This gate receives the ${inputRelease} review-decision packet and records founder posture only. It cannot grant permission, approve authorization, execute, store, publish, or launch.`,
  `This founder desk receives the ${inputRelease} review-decision packet and records one posture only. It cannot grant permission, approve authorization, execute, store, publish, or launch.`
);
page = page.replace(
  "Draft-only, hold, or reject without authority.",
  "Draft-only, hold, return, or reject without authority."
);
page = page.replace(`${inputRelease} review decision`, `${inputRelease} review decision`);
...
const cssBlock = `
/* VEDAPATH v3.8.7 FOUNDER DECISION RE-ENTRY */
body.vp-command-shell-ready.permission-execution-decision-page main.workspace {
  grid-template-columns: minmax(184px, 0.42fr) minmax(0, 3.08fr) minmax(190px, 0.46fr);
  gap: 18px;
}
...
body.permission-execution-decision-page .founder-decision {
  ...
  background:
    radial-gradient(circle at 94% 6%, rgba(20, 92, 74, 0.06), transparent 25%),
    linear-gradient(180deg, rgba(255, 255, 253, 0.99), rgba(255, 253, 249, 0.96));
...
body.permission-execution-decision-page .founder-decision-list,
body.permission-execution-decision-page .founder-decision-rules {
  gap: 9px;
}
...
body.permission-execution-decision-page .founder-decision-result h2,
...
`;
if (!css.includes("VEDAPATH v3.8.7 FOUNDER DECISION RE-ENTRY")) {
  css = `${css.trim()}\n\n${cssBlock.trim()}\n`;
...
commandShell = commandShell.replace('const releaseBadge = "v3.8.6 decision";', `const releaseBadge = "${releaseBadge}";`);
if (!commandShell.includes(`const releaseBadge = "${releaseBadge}";`)) {
  throw new Error("Missing command shell release badge anchor");
}
write("assets/vedapath-command-shell.js", commandShell);

let build = read("build-status.html");
build = build.replaceAll("v3.8.6 decision", releaseBadge);
build = replaceRequired(build, "<strong>v3.8.6</strong>", `<strong>${release}</strong>`, "build current version");
build = replaceRequired(
  build,
  "<p>Controlled Permission Execution Authorization Review Decision Gate Re-entry: the decision gate now receives the v3.8.5 draft-review packet and routes only to founder decision, hold, return, or block while every authority flag stays false.</p>",
...
build = replaceRequired(
  build,
  "<p>The trust chain now routes the v3.8.5 draft-review packet without turning route readiness into permission, authorization, execution, storage, public release, or production.</p>",
...
const oldPhase347 = `<article class="phase">
...
const newPhase347 = `<article class="phase">
...
build = replaceRequired(build, oldPhase347, newPhase347, "phase 347");
build = replaceRequired(
  build,
  '<div class="version-row"><span>Release</span><strong>v3.8.6 Controlled Permission Execution Authorization Review Decision Gate Re-entry</strong></div>',
  `<div class="version-row"><span>Release</span><strong>${release} ${releaseName}</strong></div>`
);
build = replaceRequired(
  build,
  '<div class="version-row"><span>Previous</span><strong>v3.8.5 Controlled Permission Execution Authorization Draft Review Gate Re-entry</strong></div>',
  `<div class="version-row"><span>Previous</span><strong>${previousRelease}</strong></div>`
);
build = replaceRequired(
  build,
  "<div class=\"version-row\"><span>Goal</span><strong>Route the v3.8.5 draft-review packet to founder decision, hold, return, or block without granting permission, approving authorization, or enabling execution.</strong></div>",
...
readme = insertBefore(readme, "## v3.8.6 Controlled Permission Execution Authorization Review Decision Gate Re-entry", readmeEntry);
...
prototypeNotes = insertBefore(prototypeNotes, "## v3.8.6 Controlled Permission Execution Authorization Review Decision Gate Re-entry", notesEntry);
write("docs/PROTOTYPE_NOTES.md", prototypeNotes);

const blueprintEntry = `### 360. Founder Permission Execution Authorization Decision Gate Re-entry

Founder Permission Execution Authorization Decision Gate Re-entry should receive the ${inputRelease} review-decision packet and record founder posture as draft-only, hold, return, or reject while every authority flag remains false.

It should:

- accept only the ${inputRelease} controlled permission execution authorization review-decision output
