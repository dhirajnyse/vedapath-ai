...
const release = "v3.9.6";
const inputRelease = "v3.9.5";
const releaseBadge = "v3.9.6 draft";
const releaseName = "Controlled Permission Execution Authorization Draft Gate Re-entry";
const previousRelease = "v3.9.5 Founder Permission Execution Authorization Decision Gate Re-entry";
const nextRelease = "v3.9.7 Controlled Permission Execution Authorization Draft Review Gate Re-entry";
...
draftData.schema_version = "controlled-permission-execution-authorization-draft-gate-v7";
draftData.release = release;
draftData.generated_at = generatedAt;
draftData.title = releaseName;
draftData.summary = `Re-enters the controlled draft gate from the ${inputRelease} founder posture packet, preserves source identity, founder posture id, route, questions, and authority audit, and prepares one reviewable draft while every permission, authorization, execution, storage, public release, and production path remains disabled.`;
...
let js = read(jsFile);
js = replaceAllIfPresent(js, "v3.9.1", inputRelease);
js = replaceAllIfPresent(js, "v3.9.2", release);
js = replaceAllIfPresent(js, "controlled-permission-execution-authorization-draft-gate-v6", "controlled-permission-execution-authorization-draft-gate-v7");
write(jsFile, js);

let html = read(pageFile);
html = replaceAllIfPresent(html, "v3.9.2 draft", releaseBadge);
html = replaceAllIfPresent(html, "v3.9.2", release);
html = replaceAllIfPresent(html, "v3.9.1", inputRelease);
html = replaceAllIfPresent(html, "Draft one quiet packet.", "Carry one posture into review.");
html = replaceAllIfPresent(html, "Quiet draft desk", "Controlled writing desk");
html = replaceAllIfPresent(html, "One posture. One draft. No authority.", "One posture. One draft. No power opens.");
html = replaceAllIfPresent(
  html,
  `VedaPath carries the ${inputRelease} founder posture into one reviewable draft while keeping route, source ids, questions, and false authority audit intact.`,
...
html = replaceAllIfPresent(
  html,
  "This gate receives the v3.9.5 founder posture, keeps source identity intact, and prepares one draft-review candidate only.",
  "This gate receives the v3.9.5 founder posture, keeps source identity intact, and prepares one reviewable draft only."
);
html = replaceAllIfPresent(
  html,
  "One prepared draft can move to review. The locks stay visible and closed.",
  "One prepared draft can move to review. Every real authority lock stays visible and closed."
);
write(pageFile, html);

let css = read(cssFile);
const cssBlock = `
/* VEDAPATH v3.9.6 CONTROLLED DRAFT GATE RE-ENTRY */
...
body.permission-execution-draft-page .draft-gate {
  ...
  background:
    radial-gradient(circle at 92% 8%, rgba(224, 168, 59, 0.13), transparent 28%),
    linear-gradient(180deg, rgba(255, 255, 253, 0.98), rgba(255, 251, 246, 0.96));
}
...
body.permission-execution-draft-page .draft-gate .muted,
body.permission-execution-draft-page .panel .muted {
  max-width: 820px;
}
...
body.permission-execution-draft-page .draft-gate-mark {
  ...
  height: 72px;
  align-self: start;
  box-shadow: 0 16px 36px rgba(168, 62, 18, 0.1);
}
...
body.permission-execution-draft-page .metric {
  border-color: rgba(73, 87, 72, 0.14);
  background: rgba(255, 255, 253, 0.9);
  box-shadow: none;
}

body.permission-execution-draft-page .draft-step span {
  background: rgba(20, 92, 74, 0.11);
  color: #145c4a;
}
...
`;
if (!css.includes("VEDAPATH v3.9.6 CONTROLLED DRAFT GATE RE-ENTRY")) {
  css = `${css.trim()}\n\n${cssBlock.trim()}\n`;
}
write(cssFile, css);
...
build = replaceRequired(build, '<span class="version">v3.9.5 founder</span>', `<span class="version">${releaseBadge}</span>`, "build version badge");
build = replaceRequired(
  build,
  `<strong>v3.9.5</strong>
          <p>Founder Permission Execution Authorization Decision Gate Re-entry: the founder gate receives the v3.9.4 review-decision packet and records only founder posture while permission, authorization, execution, storage, public release, and production stay false.</p>`,
  `<strong>${release}</strong>
          <p>${releaseName}: the draft gate receives the ${inputRelease} founder posture packet and prepares one reviewable draft while permission, authorization, execution, storage, public release, and production stay false.</p>`,
  "build current version card"
);
build = replaceRequired(
  build,
  "The trust chain now records founder posture from the v3.9.4 review-decision packet while preserving question handoff, source identity, authority flag audit, and all false execution flags.",
  `The trust chain now carries the ${inputRelease} founder posture into controlled draft language while preserving question handoff, source identity, authority flag audit, and all false execution flags.`,
  "build full vision copy"
);
build = replaceRequired(
  build,
  `<strong>Controlled Permission Execution Authorization Draft Gate Re-entry</strong>
          <p>Receive the v3.9.5 founder posture packet and prepare draft-only controlled language while every authority flag remains false.</p>`,
...
build = replaceRequired(
  ...
  `<article class="phase">
        ...
        <div>
          <strong>Phase 356: Controlled Permission Execution Authorization Draft Gate Re-entry</strong>
          <p>Receive the v3.9.5 founder posture packet and prepare draft-only controlled language while every authority flag remains false.</p>
        </div>
  ...
  `<article class="phase">
        ...
        <div>
          <strong>Phase 356: Controlled Permission Execution Authorization Draft Gate Re-entry</strong>
          <p>Receives the ${inputRelease} founder posture packet and prepares controlled draft-review candidate language while every authority flag remains false.</p>
        </div>
  ...
  "build phase 356 update"
);
build = replaceRequired(build, `<div class="version-row"><span>Release</span><strong>v3.9.5 Founder Permission Execution Authorization Decision Gate Re-entry</strong></div>`, `<div class="version-row"><span>Release</span><strong>${release} ${releaseName}</strong></div>`, "version release row");
build = replaceRequired(build, `<div class="version-row"><span>Previous</span><strong>v3.9.4 Controlled Permission Execution Authorization Review Decision Gate Re-entry</strong></div>`, `<div class="version-row"><span>Previous</span><strong>${previousRelease}</strong></div>`, "version previous row");
build = replaceRequired(build, `<div class="version-row"><span>Goal</span><strong>Record founder posture from the v3.9.4 review-decision packet while every authority flag remains false.</strong></div>`, `<div class="version-row"><span>Goal</span><strong>Prepare controlled draft-review candidate language from the ${inputRelease} founder posture packet while every authority flag remains false.</strong></div>`, "version goal row");
...
build = replaceRequired(
  ...
  `<li><span class="dot"></span><span>Re-enter controlled permission execution authorization draft gate.</span></li>
              <li><span class="dot"></span><span>Receive the v3.9.5 founder posture packet without granting authorization or execution.</span></li>
...
- Polishes the draft page into a quiet writing desk: one posture, one reviewable draft, visible locks, and the ${releaseBadge} command-shell badge.
...
readme = insertBefore(readme, "## v3.9.5 Founder Permission Execution Authorization Decision Gate Re-entry", readmeBlock, "README v3.9.5");
...
notes = insertBefore(notes, "## v3.9.5 Founder Permission Execution Authorization Decision Gate Re-entry", notesBlock, "prototype notes v3.9.5");
write(notesFile, notes);

const blueprintBlock = `## v3.9.6 Controlled Draft Gate
...
blueprint = insertBefore(blueprint, "## v3.9.5 Founder Decision Gate", blueprintBlock, "blueprint v3.9.5 marker");
write(blueprintFile, blueprint);

let draftDoc = read(draftDocFile);
draftDoc = replaceAllIfPresent(
  draftDoc,
  "Controlled Permission Execution Authorization Draft Gate Re-entry accepts the v3.9.1 founder posture packet and turns it into draft-review candidate language only.",
...
draftDoc = insertBefore(draftDoc, "## v3.9.2 Re-entry", draftDocBlock, "draft doc v3.9.2 marker");
