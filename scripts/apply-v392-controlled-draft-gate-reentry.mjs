...
const release = "v3.9.2";
const inputRelease = "v3.9.1";
const releaseBadge = "v3.9.2 draft";
const releaseName = "Controlled Permission Execution Authorization Draft Gate Re-entry";
const previousRelease = "v3.9.1 Founder Permission Execution Authorization Decision Gate Re-entry";
const nextRelease = "v3.9.3 Controlled Permission Execution Authorization Draft Review Gate Re-entry";
...
let js = read(jsFile);
js = replaceAllIfPresent(js, "v3.8.7", inputRelease);
write(jsFile, js);

let html = read(pageFile);
html = replaceAllIfPresent(html, "v3.8.8 draft", releaseBadge);
html = replaceAllIfPresent(html, "v3.8.8", release);
html = replaceAllIfPresent(html, "v3.8.7", inputRelease);
html = replaceAllIfPresent(html, "Draft one careful packet.", "Draft one quiet packet.");
html = replaceAllIfPresent(html, "Draft is not authorization", "Quiet draft boundary");
html = replaceAllIfPresent(html, "One posture, one quiet draft.", "One posture. One draft. No authority.");
html = replaceAllIfPresent(
  html,
  "VedaPath now carries the v3.9.1 founder posture into a draft-review candidate while preserving the exact route, source ids, questions, and false authority audit.",
  "VedaPath carries the v3.9.1 founder posture into one reviewable draft while keeping route, source ids, questions, and false authority audit intact."
);
html = replaceAllIfPresent(
  html,
  "A ready draft creates one later review candidate. The locks stay visible and closed.",
...
const cssBlock = `
/* VEDAPATH v3.9.2 CONTROLLED DRAFT GATE RE-ENTRY */
...
body.permission-execution-draft-page .metric {
  border-color: rgba(73, 87, 72, 0.15);
...
`;
if (!css.includes("VEDAPATH v3.9.2 CONTROLLED DRAFT GATE RE-ENTRY")) {
  css = `${css.trim()}\n\n${cssBlock.trim()}\n`;
}
write(cssFile, css);
...
build = replaceRequired(build, '<span class="version">v3.9.1 founder</span>', `<span class="version">${releaseBadge}</span>`, "build version badge");
build = replaceRequired(
  build,
  `<strong>v3.9.1</strong>
          <p>Founder Permission Execution Authorization Decision Gate Re-entry: the founder gate now receives the v3.9.0 review-decision packet and records only a draft posture while permission, authorization, execution, storage, public release, and production stay false.</p>`,
  `<strong>${release}</strong>
          <p>${releaseName}: the draft gate receives the ${inputRelease} founder posture packet and prepares one draft-review candidate while permission, authorization, execution, storage, public release, and production stay false.</p>`,
...
build = replaceRequired(
  ...
  `The trust chain now carries the ${inputRelease} founder posture into controlled draft language while preserving question handoff, authority flag audit, source ids, and all false execution flags.`,
  "build full vision copy"
);
build = replaceRequired(
  build,
  `<strong>Controlled Permission Execution Authorization Draft Gate Re-entry</strong>
          <p>Receive the v3.9.1 founder posture packet and prepare draft-only controlled language while every authority flag remains false.</p>`,
...
build = replaceRequired(
  ...
  `<article class="phase">
        ...
        <div>
          ...
          <p>Receive the v3.9.1 founder posture packet and prepare draft-only controlled language while permission, authorization, execution, storage, public release, and production remain false.</p>
        </div>
  ...
  `<article class="phase">
        ...
        <div>
          <strong>Phase 352: Controlled Permission Execution Authorization Draft Gate Re-entry</strong>
          <p>Receives the v3.9.1 founder posture packet and prepares draft-only controlled language while permission, authorization, execution, storage, public release, and production remain false.</p>
        </div>
  ...
  "build phase 352 update"
);
build = replaceRequired(build, `<div class="version-row"><span>Release</span><strong>v3.9.1 Founder Permission Execution Authorization Decision Gate Re-entry</strong></div>`, `<div class="version-row"><span>Release</span><strong>${release} ${releaseName}</strong></div>`, "version release row");
build = replaceRequired(build, `<div class="version-row"><span>Previous</span><strong>v3.9.0 Controlled Permission Execution Authorization Review Decision Gate Re-entry</strong></div>`, `<div class="version-row"><span>Previous</span><strong>${previousRelease}</strong></div>`, "version previous row");
build = replaceRequired(build, `<div class="version-row"><span>Goal</span><strong>Record a founder posture from the v3.9.0 review-decision packet while every authority flag remains false.</strong></div>`, `<div class="version-row"><span>Goal</span><strong>Prepare controlled draft-review candidate language from the ${inputRelease} founder posture packet while every authority flag remains false.</strong></div>`, "version goal row");
...
build = replaceRequired(
  ...
  `<li><span class="dot"></span><span>Re-enter controlled permission execution authorization draft gate.</span></li>
              <li><span class="dot"></span><span>Receive the v3.9.1 founder posture packet without granting authorization or execution.</span></li>
...
- Polishes the draft page into a quieter writing desk: one posture, one draft, no authority, calmer cards, and the ${releaseBadge} command-shell badge.
...
readme = insertBefore(readme, "## v3.9.1 Founder Permission Execution Authorization Decision Gate Re-entry", readmeBlock, "README v3.9.1");
...
notes = insertBefore(notes, "## v3.9.1 Founder Permission Execution Authorization Decision Gate Re-entry", notesBlock, "prototype notes v3.9.1");
write(notesFile, notes);

const blueprintBlock = `## v3.9.2 Controlled Draft Gate
...
blueprint = insertBefore(blueprint, "## v3.9.1 Founder Decision Gate", blueprintBlock, "blueprint v3.9.1 marker");
write(blueprintFile, blueprint);

let draftDoc = read(draftDocFile);
draftDoc = replaceAllIfPresent(
  draftDoc,
  "Controlled Permission Execution Authorization Draft Gate Re-entry accepts the v3.8.7 founder posture packet and turns it into draft-review candidate language only.",
...
draftDoc = insertBefore(draftDoc, "## v3.8.8 Re-entry", draftDocBlock, "draft doc v3.8.8 marker");
