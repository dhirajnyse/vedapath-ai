...
function finalChecks() {
  ...
  assert(/const releaseBadge = "(?:v4\.9\.1 controlled pilot|v4\.9\.6 pilot implementation gate|v4\.9\.7 infrastructure authorization|v4\.9\.8 deployment contract|v4\.9\.9 identity contract|v5\.(?:0\.[0-9]|1\.[01]) [^"]+)";/.test(shell), "compatible release badge");
  ...
  assert(/<strong>(?:v4\.9\.[16789]|v5\.(?:0\.[0-9]|1\.[01]))<\/strong>/.test(text("build-status.html")), "build status compatible version");
  assert(
    /implementation-ready|provider deployment,? (?:and )?invitations remain blocked|provider deployment, invitations, and public launch remain blocked|founder implementation review|private-pilot validation complete; real invitation issuance, participant access, provider operations, and public launch remain blocked/i.test(text("build-status.html")),
