...
async function v486Checks() {
  ...
  assert(/const releaseBadge = "(?:v4\.8\.6 hosted gate|v4\.9\.1 controlled pilot|v4\.9\.6 pilot implementation gate|v4\.9\.7 infrastructure authorization|v4\.9\.8 deployment contract|v4\.9\.9 identity contract|v5\.0\.0 queue migration|v5\.0\.1 pilot readiness control)";/.test(shell), "compatible release badge");
  ...
  assert(/<strong>(?:v4\.8\.6|v4\.9\.[16789]|v5\.0\.[01])<\/strong>/.test(text("build-status.html")), "build status current version");
