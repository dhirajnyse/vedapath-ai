...
const inputRelease = "v3.8.1";
const releaseBadge = "v3.8.2 decision";
const releaseName = "Controlled Permission Execution Authorization Review Decision Gate Re-entry";
const previousRelease = "v3.8.1 Controlled Permission Execution Authorization Draft Review Gate Re-entry";
...
const sampleDecision = {
  ...
  ...source,
  decision_scope: "Decide whether the v3.8.1 draft-review packet may move to founder permission execution authorization decision gate re-entry. This decision is not permission grant, not authorization approval, not execution, and cannot promote, store, update canonical records, migrate, create accounts, use secrets, publish public release, or launch production.",
  decision_language: "Review decision result: route the v3.8.1 draft-review packet to founder decision gate re-entry only. This is routing readiness only; permission grant remains closed, authorization approval remains closed, execution remains closed, and no system may run from it.",
  decision_rationale: "The v3.8.1 draft-review packet is ready, the question handoff and source identity are intact, founder posture id is preserved, evidence ids are visible, and authority stays closed. The next step is founder decision re-entry only.",
  decision_evidence_summary: "The v3.8.1 draft-review packet preserves review decision gate id, draft review gate id, draft gate id, founder decision id, founder posture id, authorization review id, preflight id, hold id, source answer id, source record id, source family, review route, founder question, permission question, authority flag audit, rollback, monitoring, stop condition, expiry, and production boundary.",
  ...
  risk_acknowledgment: "Risk remains: v3.8.1 draft-review packet mismatch, review decision id mismatch, draft review gate id mismatch, draft gate id mismatch, founder posture id mismatch, review route mismatch, founder question mismatch, permission question mismatch, authority flag audit mismatch, founder decision mismatch, authorization review mismatch, preflight mismatch, hold mismatch, source mismatch, rights change, ambiguous decision language, rollback missing, monitoring missing, packet mutation, code change, or any true permission, authorization, execution, storage, canonical, public release, or production flag must block movement.",
  rollback_condition: "Rollback if the v3.8.1 draft-review packet, route, source identity, founder posture id, questions, audit, or non-execution boundary no longer match the incoming packet.",
  monitoring_condition: "Monitor only the frozen v3.8.1 draft-review packet, route, source identity, founder posture id, questions, authority audit, and false authority flags before routing to founder decision.",
  stop_condition: "Stop if the v3.8.1 draft-review packet, review decision id, draft review gate id, draft gate id, founder posture id, review route, founder question, permission question, authority flag audit, founder decision id, authorization review id, preflight id, hold id, source ids, rights, rollback, monitoring, packet text, or code changes, or if any permission, authorization, execution, storage, canonical, public release, or production flag is true.",
  expiry_check: "Controlled permission execution authorization review decision gate re-entry expires at the next material v3.8.1 draft-review packet, review decision, draft review, draft, founder decision, authorization review, preflight, hold, source, rights, rollback, monitoring, packet, or code change and must be rechecked; not permission, not authorization, and not execution.",
  ...
  return_reason: "Return if the v3.8.1 draft-review language loses founder posture id, route, question, audit, or source identity clarity.",
  hold_reason: "Hold until the reviewer can see the exact v3.8.1 draft-review packet, founder posture id, source ids, route, questions, and authority audit.",
  block_reason: "Block if any authority flag becomes true or the v3.8.1 handoff is changed.",
  controlled_permission_execution_authorization_draft_review_ready: true,
  permission_execution_authorization_draft_review_recorded: true,
  founder_permission_execution_authorization_review_decision_candidate_ready: true,
...
data.summary = "Re-enters the review decision gate from the v3.8.1 draft-review packet, preserves founder posture id, source identity, route, questions, and authority audit, and routes founder, hold, return, or block while every permission, authorization, execution, storage, public release, and production path remains disabled.";
...
data.decision_modes = {
  ...
  "Hold for evidence": {
    ...
    summary: "Keep the packet held until the v3.8.1 draft-review evidence is visible and stable."
  },
  "Return to draft review": {
    state: "Return to draft review",
    summary: "Send the packet back to the v3.8.1 draft-review gate."
  },
  "Block packet": {
    state: "Block packet",
...
data.decision_checks = [
  { check: "Input packet", rule: "Starts only from the v3.8.1 non-authorizing draft-review output." },
...
appendIfMissing(
  cssFile,
  "VEDAPATH v3.8.2 CONTROLLED REVIEW DECISION GATE RE-ENTRY",
  `
/* VEDAPATH v3.8.2 CONTROLLED REVIEW DECISION GATE RE-ENTRY */
...
replaceAll("assets/vedapath-command-shell.js", [
  ['const releaseBadge = "v3.8.1 review";', `const releaseBadge = "${releaseBadge}";`]
]);

replaceAll("build-status.html", [
  ["v3.8.1 review", releaseBadge],
  ["<strong>v3.8.1</strong>\n          <p>Controlled Permission Execution Authorization Draft Review Gate Re-entry: the review gate now receives the v3.8.0 controlled draft packet and prepares one review-decision candidate while every authority flag stays false.</p>", `<strong>${release}</strong>\n          <p>${releaseName}: the decision gate now receives the v3.8.1 draft-review packet and routes only to founder decision, hold, return, or block while every authority flag stays false.</p>`],
  ["The trust chain now reviews the v3.8.0 controlled draft packet without turning readiness into permission, authorization, execution, storage, public release, or production.", "The trust chain now routes the v3.8.1 draft-review packet without turning route readiness into permission, authorization, execution, storage, public release, or production."],
  ["<strong>Controlled permission execution authorization review decision gate re-entry</strong>\n          <p>Route the v3.8.1 draft-review packet to review decision, hold, return, or block while every authority flag remains false.</p>", `<strong>${nextGate}</strong>\n          <p>Receive the v3.8.2 review-decision packet and record founder posture while every authority flag remains false.</p>`]
]);
...
replaceAll("build-status.html", [
  ["<div class=\"version-row\"><span>Release</span><strong>v3.8.1 Controlled Permission Execution Authorization Draft Review Gate Re-entry</strong></div>", `<div class="version-row"><span>Release</span><strong>${release} ${releaseName}</strong></div>`],
  ["<div class=\"version-row\"><span>Previous</span><strong>v3.8.0 Controlled Permission Execution Authorization Draft Gate Re-entry</strong></div>", `<div class="version-row"><span>Previous</span><strong>${previousRelease}</strong></div>`],
  ["<div class=\"version-row\"><span>Goal</span><strong>Review the v3.8.0 controlled draft packet and prepare one review-decision candidate without granting permission, approving authorization, or enabling execution.</strong></div>", "<div class=\"version-row\"><span>Goal</span><strong>Route the v3.8.1 draft-review packet to founder decision, hold, return, or block without granting permission, approving authorization, or enabling execution.</strong></div>"],
...
`;
prependBefore("README.md", "## v3.8.1 Controlled Permission Execution Authorization Draft Review Gate Re-entry", readmeEntry);
...
`;
prependBefore("docs/PROTOTYPE_NOTES.md", "## v3.8.1 Controlled Permission Execution Authorization Draft Review Gate Re-entry", notesEntry);

write(docFile, `# ${releaseName}

Controlled Permission Execution Authorization Review Decision Gate Re-entry receives the v3.8.1 controlled draft-review packet and turns it into a founder decision candidate route, hold, return, or block.
...
## v3.8.2 Re-entry Notes

- Accept only the v3.8.1 controlled draft-review packet.
...
- Preserve the v3.8.1 route, source ids, source family, questions, founder posture id, and authority audit.
